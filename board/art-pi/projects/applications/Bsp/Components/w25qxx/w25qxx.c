...
static uint16_t W25QXX_TYPE = W25Q128;//默认是W25Q128
...
{
    if(a == 0) HAL_GPIO_WritePin(SPI_FLASH_CS_GPIO_Port, SPI_FLASH_CS_Pin, GPIO_PIN_RESET);
    else       HAL_GPIO_WritePin(SPI_FLASH_CS_GPIO_Port, SPI_FLASH_CS_Pin,   GPIO_PIN_SET);
}
...
uint8_t W25QXX_Init(void)
{
    ...
    if ((W25QXX_TYPE >> 8) == 0xEF) {
        // printf("W25QXX Chip Found\r\n");
        // printf("CHIP TYPE: W25Q%d\r\n", (W25QXX_TYPE & 0xFF));
        // printf("CHIP CAPACITY: %d KByte\r\n", (1 << ((W25QXX_TYPE & 0xFF) - 13)));
        return 0;
    } else {
        // printf("W25QXX Chip Not Found\r\n");
        return 1;
    }
}
...
{
    W25QXX_CS(0);//0片选开启，1片选关闭
    SPI_ReadWriteByte(W25X_PowerDown);//发送掉电命令 0xB9
    W25QXX_CS(1);//0片选开启，1片选关闭
    delay_us(3);//等待TPD
}
...
{
    W25QXX_CS(0);//0片选开启，1片选关闭
    SPI_ReadWriteByte(W25X_ReleasePowerDown);//发送电源唤醒指令 0xAB
    W25QXX_CS(1);//0片选开启，1片选关闭
    delay_us(3);//等待TRES1
}

//延时
static void delay_us(uint32_t us) //利用CPU循环实现的非精准应用的微秒延时函数
{
    volatile uint32_t ticks = HAL_GetTick();
    while((HAL_GetTick() - ticks) < us);
}
